...
def handle_command(command: str, args: list, message=None) -> str:
    ...
    if command == "start":
        ...
        try:
            ...
            subprocess.run(["openclaw", "cron", "rm", "ralph-loop"], 
            ...
            if result.returncode == 0:
                cron_msg = f"✅ Cron set to run every {freq}"
            else:
                cron_msg = f"⚠️ Cron setup failed: {result.stderr}"
        except Exception as e:
            cron_msg = f"⚠️ Cron error: {e}"
        
        return f"🎯 **Goal set!**\n\n**Goal:** {goal}\n\n{cron_msg}\n\nRun `/ralph run` to begin. Use `/ralph stop` to halt."
    
    elif command == "stop":
        # Remove cron job when stopping
        try:
            import subprocess
            subprocess.run(["openclaw", "cron", "rm", "ralph-loop"], 
                         capture_output=True, timeout=10)
